...
runCmd() {
  ...
  if [ $su == true ]; then
    ...
    ~/rish -c "$cmd"
  elif "$HOME/adb" -s $(~/adb devices >/dev/null | grep "device$" | awk '{print $1}' | tail -1) shell "id" >/dev/null 2>&1; then
...
dependencies() {
  ...
  pkgInstall "termux-api"  # termux-api install/update
...
if ! termux-api-start &>/dev/null; then
  ...
  filePath=$(find "$Download" -type f -name "termux-api-app_v*+github.debug.apk" -print -quit)  # find downloaded Termux:API app package
  [ -f "$filePath" ] && apkInstall "$filePath"
else
  filePath=$(find "$Download" -type f -name "termux-api-app_v*+github.debug.apk" -print -quit)
  [ -f "$filePath" ] && rm -f "$filePath"
fi

# --- Create bingwall shortcut on Laucher Home ---
if [ ! -f "$HOME/.shortcuts/bingwall" ] || [ ! -f "$HOME/.termux/widget/dynamic_shortcuts/bingwall" ]; then
  ...
  if ! am start -n com.termux.widget/com.termux.widget.TermuxLaunchShortcutActivity > /dev/null 2>&1; then
    tag_name=$(curl -sL "https://api.github.com/repos/termux/termux-widget/releases/latest" | jq -r '.tag_name')
    while true; do
  ...
  # Create bingwall shortcut
  if am start -n com.termux.widget/com.termux.widget.TermuxLaunchShortcutActivity > /dev/null 2>&1; then
    [ -f "$filePath" ] && rm -f "$filePath"  # if Termux:Widget apk exist then remove it 
    echo -e "$notice Please wait few seconds! Creating bingwall shortcut to access BingWallpaper from Launcher Widget."
    mkdir -p ~/.shortcuts  # create $HOME/.shortcuts dir if it not exist
    echo -e "#!/usr/bin/bash\nbash \$PREFIX/bin/bingwall" > ~/.shortcuts/bingwall  # create bingwall shortcut script
    mkdir -p ~/.termux/widget/dynamic_shortcuts
    echo -e "#!/usr/bin/bash\nbash \$PREFIX/bin/bingwall" > ~/.termux/widget/dynamic_shortcuts/bingwall  # create bingwall dynamic shortcut script
    chmod +x ~/.termux/widget/dynamic_shortcuts/bingwall  # give execute (--x) permissions to bingwall script
    echo -e "$info From Termux:Widget app tap on ${Green}bingwall → Add to home screen${Reset}. Opening Termux:Widget app in 6 seconds.." && sleep 6
...
# Create setwall shortcut
if [ ! -f "$HOME/.shortcuts/setwall" ] || [ ! -f "$HOME/.termux/widget/dynamic_shortcuts/setwall" ]; then
  if am start -n com.termux.widget/com.termux.widget.TermuxLaunchShortcutActivity > /dev/null 2>&1; then
    echo -e "$notice Please wait few seconds! Creating setwall shortcut to Update BingWallpaper from Launcher Widget."
    echo -e "#!/usr/bin/bash\nbash \$PREFIX/bin/setwall" > ~/.shortcuts/setwall  # create setwall shortcut script
    echo -e "#!/usr/bin/bash\nbash \$PREFIX/bin/setwall" > ~/.termux/widget/dynamic_shortcuts/setwall  # create setwall dynamic shortcut script
    chmod +x ~/.termux/widget/dynamic_shortcuts/setwall  # give execute (--x) permissions to setwall script
    echo -e "$info From Termux:Widget app tap on ${Green}setwall → Add to home screen${Reset}." && sleep 5
    am start -n com.termux.widget/com.termux.widget.TermuxCreateShortcutActivity > /dev/null 2>&1  # open Termux:Widget app shortcut create activity (screen/view) to add shortcut on Launcher Home
  fi
fi
  ...
  if [ "$TERMUX_VERSION" != "$(sed 's/^v//' <<< "$tag_name")" ]; then
    ...
    if [ $su == true ]; then
      ...
      if [ "$(su -c 'getenforce 2>/dev/null')" = "Enforcing" ]; then
        ...
        touch "$BingWallpaper/setenforce0"
    ...
    if [ -f "$filePath" ]; then
      if [ $su == true ]; then
        if [ "$(su -c 'getenforce 2>/dev/null')" = "Permissive" ] && [ -f "$BingWallpaper/setenforce0" ]; then
          su -c "setenforce 1"  # set SELinux to Enforcing mode to block unauthorized operations
          rm -f "$BingWallpaper/setenforce0"
        fi
...
SchedulerUpdateWallpaper() {
  milliseconds=${1}
  termux-job-scheduler --script $BingWallpaper/SetWallpaper.sh --period-ms ${milliseconds} --job-id 1501712810 --persisted true  # ping -c 1 bing.com
}
...
TimerTriggerUpdateWallpaper() {
  ...
  (crontab -l 2>/dev/null; echo "${MM} ${HH} * * * $BingWallpaper/SetWallpaper.sh") | crontab -
...
UpdateWallpaperAtBoot() {
  if ! am start -n com.termux.boot/com.termux.boot.BootActivity &>/dev/null; then
    ...
    filePath=$(find "$Download" -type f -name "termux-boot-app_v*+github.debug.apk" -print -quit)  # find downloaded Termux:Boot app package
    [ -f "$filePath" ] && apkInstall "$filePath"
  else
  ...
  mkdir -p ~/.termux/boot/
  echo -e "#!/usr/bin/bash\nbash \$PREFIX/bin/setwall" > ~/.termux/boot/bingwall
}
